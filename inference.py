...
def generate_reports(
    ...
    for batch in tqdm(
        dataloader, desc="Generating Reports", unit="batch", total=len(dataloader)
    ):
...
def run_green_evaluation(
    ...
    for section in sections:
        ...
        try:
            ...
            print(
                f"Error in GREEN evaluation for {section} section: {str(e)}",
                file=sys.stderr,
            )
...
def main(args: argparse.Namespace):
    ...
    for split in ["test", "val"]:
        ...
        dataset = XRayReportDataset(
            args.images_dir, args.annotation_file, split=split, with_assistant=False
        )
        dataloader = DataLoader(
            dataset,
            batch_size=args.batch_size,
            shuffle=False,
            collate_fn=lambda b: XRayReportDataset.collate_fn(
                b, processor, add_generation_prompt=True
            ),
        )
        ...
        actual_reports = [
            parse_report_sections(sample["report"].strip()) for sample in dataset
        ]
        ...
        update_annotation_file(
            ...
            split,
            patient_ids,
            [report["full"] for report in generated_reports],
        )
        ...
        results_df["patient_ids"] = patient_ids
        ...
        overall_results = results_df[
            ["patient_ids"]
            + [col for col in results_df.columns if col.startswith("full_")]
        ]
        overall_results.columns = [
            col.replace("full_", "") for col in overall_results.columns
        ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="X-Ray Report Generation and Evaluation"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=".cache",
        help="Cache directory for GREEN model",
    )
    parser.add_argument(
        "--weights_dir",
        type=str,
